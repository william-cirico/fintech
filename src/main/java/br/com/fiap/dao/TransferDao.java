...
public class TransferDao implements BaseDao<Transfer, Long> {
    ...
    public Transfer insert(Transfer transfer) {
        String sql = """
                    INSERT INTO T_FIN_TRANSFER (AMOUNT, "DATE", DESCRIPTION, OBSERVATION, ORIGIN_ACCOUNT_ID, DESTINATION_ACCOUNT_ID)
                    VALUES (?, ?, ?, ?, ?, ?)
                """;
        ...
        try (Connection connection = ConnectionFactory.getConnection()) {
            try (PreparedStatement stm = connection.prepareStatement(sql)) {
                ...
                stm.setString(4, transfer.getObservation());
    ...
    public Transfer update(Transfer transfer) {
        String sql = """
                    UPDATE T_FIN_TRANSFER SET AMOUNT = ?, "DATE" = ?, DESCRIPTION = ?, OBSERVATIONS = ?, TYPE = ?, ORIGIN_ACCOUNT = ?, DESTINATION_ACCOUNT = ?
                    WHERE ID = ?
                """;
